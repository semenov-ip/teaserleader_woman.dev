...
class Sites_admin extends CI_Controller{
  ...
  function getSiteAllData(){
    ...
    return $this->setDataProcessing($this->site_pagination->select_all_row_where_foreach_column_selectcolumn_join_orderby_pagination($this->dataWhereArr, 'ls.status', 'asc', 'users lu', 'ls.user_id = lu.user_id', 'ls.site_id, ls.user_id, ls.status, ls.url, ls.stat_login, lu.email', $perPagePagination, intval($this->uri->segment(4)), 'sites ls'));
  }

  function totalRows(){
    return $this->site_pagination->select_all_row_where_foreach_column_selectcolumn_join_orderby_return_count($this->dataWhereArr, 'users lu', 'ls.user_id = lu.user_id', 'ls.site_id', 'sites ls');
  }

  function getDataWhereArr(){
    $this->dataWhereArr = ( !empty($_POST['search']) ) ? $this->search_id_url_mail->getSearchData($_POST['search']) : array();
  }
